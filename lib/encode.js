...
function createEncoder(options) {
	...
	function writeObject(object) {
				...
				if (type === 'object') {
					...
					if (constructor === Array) {
						...
						if (!writerByType[type]) {
							type = 'object' // go back to an object if we don't have a registered type
						}
					}
		...
		*/
		...
		var structureRef
		if (structureDefinition && structureIndex - (structureRef = structureDefinition.index) < 0x400) {
			...
			/*if (structureRef >= 16) {
				structureDefinition.index = propertyIndex++
				if (propertyIndex >= 0x3ff) {
					compactStructures()
				}
			}*/
...
/*					if (inLazyDocument) {
	...
	function compactProperties() {
		...
		for (var i = 0, l = keys.length; i < l; i++) {
			var key = keys[i]
			var newIndex = properties.get(key) - 0x200
			if (newIndex >= 0) {
				properties.set(key, newIndex)
			} else {
