...
function nodeCharEncoder(options) {
	...
	var offset = 0
	var end = bufferSize
	var outvar = options.outvar
	var buffer = Buffer.allocUnsafe(bufferSize)
	function makeRoom(bytesNeeded) {
		if (outvar) {
			outvar.writeBytes(buffer.slice(0, offset))
	...
	function writeObject(object) {
		...
		for (var key in object) {
			...
			if (value instanceof Array) {
				for (var i = 0, l = value.length; i < l; i++) {
					...
					if (entry && entryType === 'object' && objectTypes[entry.constructor.name]) {
						entryType = 'open'
					}
					if (entryType !== type) {
						if (!type) {
			...
			if (property) {
				if (property.writer !== writer) {
					var propertyForType = property[type]
					if (propertyForType) {
						property = propertyForType
					} else {
						...
						(structureToWrite || (structureToWrite = []))[structure.length] = property
					}
				}
			} else {
				property = {
		...
		*/
		...
		if (structureDefinition) {
			...
			var shortRef = 0x0f & structureRef
			...
			for (var i = 0, l = structure.length; i < l; i++) {
				var property = structureToWrite[i]
				if (property) {
					var key = property.key
					writeToken(property.writer.propertyCode, key.length)
					writeString(key)
				} else {
					writeToken(0, startingPropertyLength - structureIndices[i])
				}
			}
		}
		for (var i = 0, l = structure.length; i < l; i++) {
			currentProperty = structure[i]
			currentProperty.writer(object[currentProperty.key])
		}
	}
