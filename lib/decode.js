...
/* The ascii variant
	...
	function objectByReference(reference) {
		if (reference < 16)
			return objectWithStructure(lastReferencedStructures[reference], {})
		else {
	...
	function objectByStructureReference(structureIndex) {
		...
		return objectWithStructure(lastReferencedStructures[0x0f & structureIndex] = structures[structureRef], {})
	}
...
var decodeOnDemand = {
	...
	getDecoded(target) {
		...
		return target.decodedDPack = decoder.setSource(primaryBuffer.toString(), 0).readOpen()
	},
	ownKeys(target) {
