...
function nodeCharEncoder(options) {
	...
	function writeBufferAtBeginning(headerBuffer, startIndex) {
		var headerLength = headerBuffer.length
		if (offset + headerLength > end) {
			makeRoom(headerLength)
		}
		buffer.copy(buffer, headerLength, startIndex, offset)
...
function createSerializer(options) {
					...
					if (constructor === Array) {
						...
						if (writerByConstructor.has(constructor)) {
							...
							if (!writerByType[type]) {
								...
								writerByType[type].propertyCode = 8
							}
						}
					}
	...
	var otherWrites = {
		...
		object: function(object) {
			if (object) {
				var constructorWriter = writerByConstructor.get(object.constructor)
				if (constructorWriter) {
					if (constructorWriter.propertyCode > -1) {
						writeToken(0, constructorWriter.propertyCode + 7)
						if (constructorWriter.propertyCode > 3) {
							writeToken(0, 10)
							writeOther(object.constructor.name)
						}
					}// else an array, probably, which writes itself
...
/*					if (inLazyDocument) {
	...
	function writeBlock(block, writer) {
		...
		var lazyPromise = {
			then: function(callback) {
				const startOffset = charEncoder.offset
				writeToken(0, 14)
				writeOther(id)
	...
	return {
		serialize: function(value) {
			...
			if (options.withLength || pendingEncodings.length > 0 && !options.outlet) {
				...
				charEncoder.writeBufferAtBeginning(headerEncoder.getSerialized(), 1) // omit the "start-block" token
			}
		},
		startBlock: function() {
			writeToken(0, 12) // start block
		},
...
const parsedSymbol = require('./Block').parsedSymbol
