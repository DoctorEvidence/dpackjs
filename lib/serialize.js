...
function createSerializer(options) {
	...
	var canUpdateShared = !options.outlet && !options.noShareUpdate
		...
		if (values) {
			...
			if (reference > -1) {
				...
				if (property.writeSharedValue) {
					if (property.writeSharedValue(value, writeToken, canUpdateShared))
						return
				} else {
		...
		for (var key in object) {
			...
			if (!property || property.key !== key ||
				...
				if (property) {
					...
					property = thisProperty.getProperty(value, key, type, extendedType, writeProperty, writeToken, lastPropertyIndex, canUpdateShared)
			...
			for (var i = 0; i < length; i++) {
				...
				if (!property) {
					if (arrayProperty.getProperty) {
						// a shared property
						property = arrayProperty.getProperty(value, null, type, extendedType, writeProperty, writeToken, 0, canUpdateShared)
					...
					if (arrayProperty.getProperty) {
						// a shared property
						property = arrayProperty.getProperty(value, null, type, extendedType, writeProperty, writeToken, ++propertyIndex, canUpdateShared)
					} else {
						writeToken(PROPERTY_CODE, ++propertyIndex)
	...
	var serializer = {
		serialize: function(value, sharedProperty) {
			var buffer = value && value[bufferSymbol] && value[bufferSymbol](sharedProperty, !(options && options.noShareUpdate))
			if (buffer) {
