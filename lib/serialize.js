...
//				resumeIndex = property.resumeIndex
	...
	var serializer = {
		serialize: function(value, sharedProperty) {
			...
			if (buffer) {
				charEncoder.writeString(buffer)
				return
			}
			if (sharedProperty) {
...
function serialize(value, options) {
	...
	if (options && options.lazy) {
		let sizeTable = value[exports.sizeTableSymbol]
		return sizeTable ? '\x7f' + serialize(sizeTable) + serialized : serialized
	}
	return serialized
}
...
function browserCharEncoder() {
	...
	return {
		...
		//insertBuffer,
		flush: function(specifiedOutlet) {
			(specifiedOutlet || outlet).writeBytes(serialized)
			serialized = ''
		},
