...
var EXTENSIONS = 8
var METADATA_TYPE = 9
var KEY_RENAME = 10 // is this needed?
var DEFERRED_REFERENCE = 11
...
function createSerializer(options) {
	...
	function writeProperty(value, key, type, extendedType, index) {
		...
		if (type === 'string') {
			...
			property[0].writeValue = writeAsDefault
	...
	function writeAsArray(array, parentProperty) {
		if (array && array.constructor === Array) { // check to make sure it is an array
			...
			if (typeof array[0] === 'string' && property.type !== 'string') {
				property = arrayProperty[0] = writeProperty('', null, 'string')
			}
			// write out the elements
			for (var i = 0; i < length; i++) {
				// TODO: If a change in property is required, give it a new index slot
				property.writeValue(array[i], arrayProperty)
			}
			if (property !== arrayProperty[0]) {
