...
function createSerializer(options) {
	...
	var property
	if (options.shared && !options.shared.writeValue) {
		var types = [
			0, 1, 2, 3, 4 , 5,
			writeAsDefault,
			writeAsArray,
		...
		var addWriters = function(property) {
			property.writeValue = types[property.type]
			for (var i = 0; i < property.length; i++) {
				addWriters(property[i])
			}
		}
		addWriters(options.shared)
	}
	...
	function writeAsReferencing(value) {
		...
		if (values) {
			...
			if (reference > -1) {
				...
				if (property.writeSharedValue(value))
					return
			} else {
		...
		for (var key in object) {
			...
			if (!property || property.key !== key ||
				...
				if (property) {
					...
					debugger
					propertyIndex = thisProperty.newId(value, key, type, extendedType, writeProperty, writeToken, lastPropertyIndex)
	...
	function writeProperty(value, key, type, extendedType, index) {
		...
		if (extendedType) {
			if (extendedType.createPropertyWriter) {
				property.writeValue = extendedType.createPropertyWriter(charEncoder, index)
			} else {
				property.extendedType = extendedType
				writeToken(TYPE_CODE, METADATA_TYPE)
				writeInlineString(extendedType.name)
		...
		if (!array) {
			...
			for (var i = 0; i < length; i++) {
				...
				if (!property) {
					...
					(extendedType && property.extendedType !== constructor)) {
					property = arrayProperty[0] = writeProperty(value, null, type, extendedType, 0)
				}
				property.writeValue(value)
			}
			if (needsClosing) {
