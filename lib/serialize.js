...
function createSerializer(options) {
	...
	function writeAsReferencing(value) {
		...
		if (values) {
			...
			} else if ((type = typeof value) === 'string' || type ==='object' && value) {
				if (property.writeSharedValue) {
					if (property.writeSharedValue(value, writeToken, avoidShareUpdate))
						return
				} else {
		...
		for (var key in object) {
			...
			if (!property || property.key !== key ||
				...
				if (property) {
					...
					property = thisProperty.getProperty(value, key, type, extendedType, writeProperty, writeToken, lastPropertyIndex, avoidShareUpdate)
	...
	function writeAsArray(array, parentProperty) {
		if (!array) {
			...
			for (var i = 0; i < length; i++) {
				...
				} else if ((property.type !== type && type !== 'boolean' && type !== 'undefined') ||
					(extendedType && property.extendedType !== constructor)) {
					propertyIndex = -1
					do {
						property = arrayProperty[++propertyIndex]
					} while(property && ((property.type !== type && type !== 'boolean' && type !== 'undefined') ||
							(extendedType && property.extendedType !== constructor)))
					if (property) {
						writeToken(PROPERTY_CODE, propertyIndex)
					} else if (arrayProperty.getProperty) {
						// a shared property
						property = arrayProperty.getProperty(value, null, type, extendedType, writeProperty, writeToken, propertyIndex)
					} else {
						writeToken(PROPERTY_CODE, propertyIndex)
						property = writeProperty(value, null, type, extendedType, propertyIndex)
	...
	function writeBlockReference(block, writer) {
		...
		function then(callback) {
			if (options.forBlock) {
				...
				var buffer = block[bufferSymbol] && block[bufferSymbol](blockProperty)
				if (buffer) {
					var shared = block[sharedSymbol]
					if (shared && !shared.upgrade(blockProperty)) {
						// shared upgrade needs to be written out
						writeBuffer(shared.serialized)
					}
					writeBuffer(buffer)
				} else {
...
function serialize(value, options) {
	...
	var buffer
	if (sharedProperty) {
		// record the start and end of the write
		sharedProperty.startWrite(options.avoidShareUpdate)
		try {
			serializer.serialize(value, sharedProperty)
			buffer = serializer.getSerialized()
		} finally {
			sharedProperty.endWrite()
		}
	} else {
		serializer.serialize(value, sharedProperty)
		buffer = serializer.getSerialized()		
...
var sharedSymbol = require('./Block').sharedSymbol
