...
const TRUE = 2
const FALSE = 3
global.propertyReferences = 0
global.typeDuplicates = 0
global.propertyCreations = 0
...
function createSerializer(options) {
	...
	function writeAsString(value) {
		...
		if (type === 'string') {
			var reference = property.strings && property.strings.get(value)
			if (reference > -1) {
				writeNumber(reference + 4)
			} else {
				property.strings.set(value, nextStringId++)
	...
	var serializer = {
		serialize: function(value) {
			...
			if (pendingEncodings.length > 0 && !options.outlet) {
				...
				headerEncoder.writeToken(SEQUENCE_CODE, pendingEncodings.length + 4) // length-defined value
				headerEncoder.writeToken(PROPERTY_CODE, 0) // null id
				headerEncoder.writeToken(PROPERTY_CODE, 0) // null imports
				headerEncoder.writeToken(NUMBER_CODE, 0) // unknown length
				headerEncoder.writeToken(SEQUENCE_CODE, 4) // the primary block (one element)
				headerEncoder.writeToken(PROPERTY_CODE, 0) // null id
				headerEncoder.writeToken(PROPERTY_CODE, 0) // null imports
