...
function createSerializer(options) {
	...
	function writeBlockReference(block, writer) {
		...
		if (shared && property.sharedBlock != shared) {
			if (property.compatibleBlockShare && property.compatibleBlockShare(shared)) {
				// if it is compatible, don't need to write it out again
			} else {
				var buffer = shared.serialized
				if (buffer.length > 1) {
					writeToken(TYPE_CODE, TYPE_DEFINITION)
					writeBuffer(shared.serialized)
				}
			}
			property.sharedBlock = shared
		}
