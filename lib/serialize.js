...
function createSerializer(options) {
	...
	function writeAsTypedValue(value) {
		if (typeof value === 'object') {
			if (value) {
				...
				return writeAsObject(value, 0, true) // indicate that it must use typed form
			} else
				return writeNull()
		}
		writeToken(TYPED_ARRAY_CODE, 2) // typed array with 2 values, the type and the value
	...
	function writeAsObject(object, index, type) {
		if (!object) {
			writeToken(0, 0)
			return
		}
		...
		for (var i = 0; i < length; i++) {
			...
			var type = typeof value
			var property = currentType[i]
			if (!property || property.key !== key || property.type !== type) {
				var property = propertyMap.get(key)
				if (property.type === type) { // right type, we can just reference the property
