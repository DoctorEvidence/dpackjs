...
function createSerializer(options) {
	...
	function writeAsReferencing(value) {
		...
		if (values) {
			if (values.resetTo > -1 && values.serializer !== serializer.id) {
				values.length = values.resetTo
				values.serializer = serializer.id
	...
	function writeAsDefault(value, parentProperty) {
		...
		if (object[targetSymbol]) {
			...
			property = writeProperty(value, property.key, 'array')
			...
			if (typeof Symbol !== 'undefined' && object[Symbol.iterator]) {
				writeProperty(value, property.key, 'array')
				writeAsIterable(object)
			}
			notPlainObject = true
		}
		var thisProperty = property
		if (thisProperty.resetTo < thisProperty.length && thisProperty.serializer != serializer.id) {
			thisProperty.length = thisProperty.resetTo
			thisProperty.serializer = serializer.id
		}
		startSequence()
		var i = 0
		...
		for (var key in object) {
			...
			if (propertyUsed)
				propertyUsed(property, object, serializer)
	...
	function writeAsArray(array, parentProperty) {
		if (!array) {
			...
			for (var i = 0; i < length; i++) {
				...
				if (propertyUsed)
					propertyUsed(property, array, serializer)
				var code = property.code
				if (code > 7) {
					if (code === 8)
						writeAsReferencing(value)
