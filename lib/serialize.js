...
function createSerializer(options) {
	...
	function writeAsReferencing(value) {
		...
		if (type === 'string') {
			...
			return writeProperty(value, null, 'number').writeValue(value)
		} else {
			writeAsDefault.call(property, value)
		}
	}
	...
	function writeAsDefault(value, parentProperty) {
		...
		if (constructor === Object) {
			...
			if (object.then || constructor === serialize.Block) {
		...
		var writeValue = createPropertyWriter(property, true)
		startSequence()
		var i = 0
		for (var key in object) {
			if (notPlainObject && !object.hasOwnProperty(key))
				continue
			writeValue(object[key], key)
			i++
		}
		endSequence(i)
	}
	function createPropertyWriter(thisProperty, isObject) {
		var propertyIndex = 0
		// densely packed property references can be searched more quickly
		var sparse = false
		...
		return function(value, key) {
			...
			if (!property || property.key !== key ||
				...
				if (property) {
					...
					// we can define the property in the next slot without having to specify an index
					if (lastPropertyIndex === 0 && !isObject && type === 'object') {
						// the default property value for an array, don't need to write it out
						property = []
					...
					thisProperty[propertyIndex = lastPropertyIndex] = property
				} else {
	...
	function writeAsArray(array, parentProperty) {
		if (array && array.constructor === Array) { // check to make sure it is an array
			...
			var writeValue = createPropertyWriter(this, false)
			// write out the elements
			for (var i = 0; i < length; i++) {
				writeValue(array[i], null)
			}
			if (needsClosing) {
		...
		function then(callback) {
			if (options.forBlock) {
				...
				if (buffer) {
					...
					writeAsDefault.call(blockProperty, block) // write it out as the next block
				}
			}
			callback()
		}
...
function serialize(value, options) {
	...
	buffer.sizeTable = value && value[sizeTableSymbol]
