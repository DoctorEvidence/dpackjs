...
function createSerializer(options) {
	...
	var lastReference = 0
	var nextBlockId = 16
	var nextObjectPropertyId = 16
	var nextPropertyIndex = 8
	var property = { index: 0, refIndex: 1 } // default root property
	var parentProperty = { index: 0 }
	...
	function writeAsString(value) {
		...
		if (type === 'string') {
			var strings = parentProperty.strings || (parentProperty.strings = [])
			if (strings) {
				var reference = strings.indexOf(value)
				if (reference > -1) {
					writeNumber((reference ^ property.index) + 4)
				} else {
	...
	function writeTypedNonConstant(value) {
		...
		if (type === 'object') {
			if (value) {
				...
				type = 'undefined' // treat null as same type as undefined, both varants
			}
		} else if (type === 'boolean') {
			type = 'undefined'
		}
		writeProperty(value, null, type, extendedType, 0).writeValue(value)
	}
				...
				if (property) {
					if (propertyIndices[property.index] === property) {
						if (property.type === type && (!extendedType || property.extendedType === constructor)) { // right type, we can just reference the property
							...
							propertyIndices[property.index] = property
						}
					} else {
						parentProperty = property
	...
	function writeProperty(value, key, type, extendedType) {
		var index = type === 'object' ? nextObjectPropertyId += 16 : createRelativeReference(parentProperty)
		property = {
			type: type,
			index: index,
			refIndex: 1,
		...
		if (type === 'string') {
			...
			throw new Error('Unable to write value of type ' + type)
		}
	...
	function getRelativeReference(parent, reference) {
		return parent.index ^ reference
	}

	function createRelativeReference(parent) {
		var position = parent.refIndex++
		return parent.index ^ (position & 15)
	}
...
/*					if (inLazyDocument) {
					...
					*/
					...
					if (buffer) { // block array
						writeToken(SEQUENCE_CODE, BLOCK_LENGTH_SUB_CODE) // indicate it is a block
					...
					writeToken(PROPERTY_CODE, getRelativeReference(id)) // identify the block
	...
	function writeBlockReference(block, writer) {
		...
		var lazyPromise = {
			then: function(callback) {
				...
				writeToken(SEQUENCE_CODE, id) // identify the block
				if (buffer) { // block array
					writeToken(SEQUENCE_CODE, BLOCK_LENGTH_SUB_CODE) // indicate it is a block with length
					writeAsDefault(buffer.length) // write length
					writeBuffer(buffer)
				} else {
				}
