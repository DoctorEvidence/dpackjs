...
function createSerializer(options) {
	...
	function writeArray(number, writer) {
		var l = array.length
	...
	function writeAsDefault(value) {
		...
		if (type === 'number' && (value >>> 0 === value || (value > 0 && value < 0x400000000000 && value % 1 === 0))) {
			...
			writeToken(NUMBER_CODE, value ? TRUE : UNDEFINED)
		} else if (type ==='string') {
			writeInlineString(value)
	...
	function writeAsObject(object, type) {
		...
		for (var key in object) {
			...
			var propertyIndex = strings.get(key)
			if (!propertyIndex) {
				strings.set(key, propertyIndex = nextStringId++)
			}
			structureIndices.push(propertyIndex)
			values.push(value)
		}
				...
				if (constructor === Array) {
					if (value.length > 0) {
						...
						if (first && typeof first === 'object') {
							...
							return writeAsArrayOf(writeType(value))
						}
	...
	function writeObject(object) {
		...
		if (structureDefinition) {
			...
			for (var ki = 0, kl = keys.length; ki < kl; ki++) {
				var key = keys[ki]
