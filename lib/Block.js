...
specialGetters[bufferSymbol] = function() {
	return function(property, randomAccess) {
		...
		if (!this.shared && propertyAsBlockStructure) {
			if (this.dpackBuffer) {
				var encoder = nodeCharEncoder({})
				var property = property.parent.getProperty(null, property.key, 'unshared', null, () => {
					encoder.writeToken(3, 6)
				}, encoder.writeToken)
				this.sizeTableBuffer = null
				return Buffer.from(encoder.getSerialized(), this.dpackBuffer)
			} else
...
function getSerialized(target, shareProperty) {
	...
	var serializerOptions = {
		forBlock: function(block, property) {
			...
			if (property && property.mustSequence) {
				mustSequence = true
