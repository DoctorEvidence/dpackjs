...
function getParsed(target) {
	...
	if (firstByte === 0x4d) { // must start with a length definition for lazy evaluation
		...
		while (offset < buffer.length) {
			...
			if (firstByte === 0x4c) { // must have an being-block-with-id token
				...
				var nextBuffer = buffer.slice(offset + afterIdOffset + 1, offset += parser.getOffset() + length + 1)
				referenceableValues[id] = new Proxy({
					buffer: nextBuffer,
					parser: parser
				}, binaryMapped)
				offset++ // skip the end-block-token
			} else {
