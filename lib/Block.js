...
exports.copy = function(source) {
	return new Proxy(target[parsedSymbol] ? {
		parsed: target[parsedSymbol],
		buffer: target[bufferSymbol],
		sizeTableBuffer: target[sizeTableSymbol]
	} : {
		parsed: target,
	}, copyOnWriteHandler)
}
...
function getParsed(target) {
	...
	while (offset < totalSizeTableLength) {
		...
		if (type < 2) { // leaf node
			...
			sizeTableLength = readUInt16BE(offset) & 0x3fff
			dpackLength = readUInt16BE(offset + 2)
		} else {
			sizeTableLength = readUInt32BE(offset) & 0x3fffffff
			dpackLength = readUIntBE(offset + 4, 6)
		}
...
function getSerialized(target) {
		...
		forBlock(block) {
			...
			if (!sizeTableBuffer) {
				...
				if (bufferLength < 64) {
					...
					sizeTableBuffer.writeUInt32BE(0xb0000010) // binary-11 and then indicate a size of 16
	...
	if (dpackBuffer.length >= 0x10000) { // || ourSizeBlock.length > 0x4000
		ourSizeBlock.writeUInt32BE(ourSizeBlock.length + 0xb0000000, 0) // binary-11 and then 30 bits
