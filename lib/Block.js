...
var copyOnWriteHandler = {
	get: function(target, key) {
		...
		if (value && value[bufferSymbol]) {
			...
			copied[key] = value = new Proxy({
				dpackBuffer: value[bufferSymbol],
				sizeTableBuffer: value[sizeTableSymbol],
				parsed: value,
				parent: target
			}, copyOnWriteHandler)
		} else if (copied) {
	...
	changed: function(target) {
		if (target.hasOwnProperty('dpackBuffer')) {
...
exports.copy = function(source) {
	return new Proxy(source[parsedSymbol] ? {
		parsed: source[parsedSymbol],
		dpackBuffer: source[bufferSymbol],
		sizeTableBuffer: source[sizeTableSymbol]
	} : {
		parsed: source,
	}, copyOnWriteHandler)
}
