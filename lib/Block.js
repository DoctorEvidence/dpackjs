...
specialGetters[bufferSymbol] = function() {
	return function(property, canUpdate) {
		var propertyAsBlockStructure = property && property.asBlockStructure
		if (!this.shared && propertyAsBlockStructure && !this.dpackBuffer)
			this.shared = propertyAsBlockStructure.call(property)
		var buffer = this.dpackBuffer || getSerialized(this, canUpdate).dpackBuffer
		if (this.shared || propertyAsBlockStructure)
			return serializeSharedBlock(buffer, this.shared, property, canUpdate)
		return buffer
	}.bind(this)
}
...
function getSerialized(target, canUpdate) {
	var childBlocks = []
	var childSizeTables = []
	var childDpackSizes = 0
	var serializerOptions = {
		forBlock: function(block, property) {
			var dpackBuffer = block[bufferSymbol](property, canUpdate)
