...
exports.makeBlockFromBuffer = function(buffer, shared) {
...
specialGetters[bufferSymbol] = function() {
	return function(property, randomAccess) {
		var propertyAsBlockStructure = property && property.asBlockStructure

		if (!this.shared) {
			if (propertyAsBlockStructure) {
				this.shared = propertyAsBlockStructure.call(property)
				this.dpackBuffer = null
				// TODO: In this case, since we know we are generating a compatible upgrade, we could shortcut some of the logic in upgrade
			} else {
				return this.dpackBuffer || getSerialized(this, null).dpackBuffer
			}
		}
		if (!this.dpackBuffer) {
			getSerialized(this, this.shared)
		}
		if (this.shared && !this.shared.upgrade(property, randomAccess)) {
			// if the property upgrade was incompatible, we have to reserialize
			if (randomAccess && property && property.recordUpdate) {
				getSerialized(this, property).dpackBuffer
				var specificBuffer = this.dpackBuffer
				this.dpackBuffer = null
				return specificBuffer
			} else // or if we are not in a random access block with a share, we can just prefix with the shared serialization
				return Buffer.concat([this.shared.serialized, this.dpackBuffer])
		}
...
function copyWithParent(source, parent) {
	...
	return new Proxy({
		...
		get dpackBuffer() {
			return source[bufferSymbol](source[sharedSymbol])
		},
	...
	return target.parsed = parse(rootBlock, childDpackBlocks.length > 0 ? { // if no child blocks, use normal deferred parsing
		...
		forDeferred: function(value, property) {
			return new Proxy({
				...
				shared: property && property.asBlockStructure && property.asBlockStructure()
			}, onDemandHandler)
		},
		freezeObjects: process.env.NODE_ENV != 'production'
	} : {
...
function getSerialized(target, shareProperty) {
	...
	var serializerOptions = {
		forBlock: function(block, property) {
			if (property.mustSequence) // it is only a follow-up to a mustSequence that forces us to abandon the size table
				mustSequence = true
