...
specialGetters[bufferSymbol] = function() {
	return function(property, randomAccess) {
		...
		if (!(this.shared && this.shared.upgrade) && propertyAsBlockStructure) {
			if (this.dpackBuffer) {
				...
				var encoder = nodeCharEncoder({})
				var isArray = this.dpackBuffer[0] === 119
				encoder.writeToken(0, 1) // use the second slot (should be unused, block always has a single initial starting slot)
				if (!isArray) {
					encoder.writeToken(3, 6) // object
					encoder.writeToken(3, 0) // null key
				}
				this.sizeTableBuffer = null
				var buffer = Buffer.concat([encoder.getSerialized(), this.dpackBuffer])
		...
		if (this.shared && this.shared.upgrade && this.shared.upgrade(property, randomAccess) > 0) {
			// if the property upgrade was incompatible, we have to included the shared structure, and force sequential reading
			this.sizeTableBuffer = null
			var buffer = Buffer.concat([this.shared.serialized, this.dpackBuffer])
			buffer.mustSequence = true
			return buffer
		}
...
var nodeCharEncoder = require('./node-encoder').nodeCharEncoder