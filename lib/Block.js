...
specialGetters[bufferSymbol] = function() {
	return function(property, randomAccess) {
		var propertyIsShared = property && property.getProperty
		...
		if (!(this.shared && this.shared.upgrade) && propertyIsShared) {
			if (this.dpackBuffer) {
				...
				var buffer = this.dpackBuffer
				var writeToken = serializer.getWriters().writeToken
				if (isArray) {
					if (buffer[1] == 112)
						buffer = buffer.slice(2) // replacing the property declaration
					else
						throw new Error('Unexpected array declaration in block')
				}

				writeToken(0, 1)// use the second slot (should be unused, block always has a single initial starting slot)
				writeToken(3, isArray ? 7 : 6) // property type
				serializer.serialize(property.key)
				this.sizeTableBuffer = null
				buffer = Buffer.concat([serializer.getSerialized(), buffer])
				buffer.mustSequence = true
				return buffer
			} else
				this.shared = property
		}
		if (!this.dpackBuffer) {
