...
function nodeCharEncoder(options) {
	...
	function writeString(string) {
		...
		var bytesWritten = buffer.utf8Write(string, offset, buffer.length)
		offset += bytesWritten
	}
	...
	return {
		...
		endSequence(length) {
			...
			if (length < 11 && startOffset > -1) { // if it is short enough, and hasn't been cleared, we can set the beginning byte length
				buffer[startOffset] = 80 + length
				return
			}
			buffer[offset++] = 93 // else we need to put an end sequence token in
		},
		getOffset() {
			return offset
		},
