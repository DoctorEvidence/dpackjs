...
function nodeCharEncoder(options) {
	...
	var encoder = {
		...
		endSequence(length) {
			...
			if (length < 15 && startOffset > -1) { // if it is short enough, and hasn't been cleared, we can set the beginning byte length
				buffer[startOffset] = 48 + length
				return
			}
			buffer[offset++] = 117 // else we need to put an end sequence token in
		},
		getOffset() {
			return offset
		},
