...
function nodeCharEncoder(options) {
	...
	function makeRoom(bytesNeeded) {
		if (outlet) {
			...
			bufferSize = Math.max(bufferSize * 4, bufferSize + bytesNeeded)
	...
	function flush(specifiedOutlet) {
		...
		buffer = Buffer.allocUnsafe(bufferSize = Math.min(bufferSize, 0x8000)) // allocate a new buffer, don't want to overwrite the bytes in the old one while they are in use!
