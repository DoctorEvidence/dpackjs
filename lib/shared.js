...
// shared: 
	...
	class Shared extends Array {
		constructor(instanceProperty) {
			...
			if (this.code == REFERENCING_TYPE) {
				...
				if (this.metadata !== UNSTRUCTURED_MARKER)
		...
		writeSharedValue(value, writeToken) {
			...
			let instanceValues = typeof this.instance.values === 'object' ? this.instance.values : (this.instance.values = [])
			let reference
			reference = instanceValues.indexOf(value)
			if (sharedValues) {
				if (reference > -1) {
					writeToken(NUMBER_CODE, reference + 12)
					return true
				}
				if (this.active !== 2) {
					this.active = 2
				...
				let length = instanceValues.length
				if (length + 12 != lastIndex) {
					// reset the reference position in the instance serialization
					writeToken(TYPE_CODE, REFERENCING_POSITION)
					writeToken(NUMBER_CODE, this.lastIndex = length + 12)
				}
				if (length < 32) {
				...
				if (reference > -1 && !currentAvoidShareUpdate) {
					this.repetitions = (this.repetitions || 0) + 1
				} else {
					// record the value so we can determine if it is repetitive
					let length = instanceValues.length
					if (length < 12) {
						instanceValues[length] = value
					}
				}
				return false // if we aren't using shared values (yet) for this property, just write string and never reference it again
			}
		}
		recordValueReference(item) {
		...
		startWrite(avoidShareUpdate, value) {
			...
			for (let i = 0; i < activeList.length; i++) {
				let activeProperty = activeList[i]
				if (activeProperty.active === 1 && isDescendant(activeProperty, this)) {
					activeProperty.active = 2
				}
			}
		...
		endWrite() {
			...
			for (let i = 0; i < activeList.length; i++) {
				...
				if (previousValues && (instance.values && instance.values.length > 0 || previousValues.size)) {
		...
		upgrade(property, randomAccess) {
			...
			if (property) {
				...
				if (property.recordUpdate) {
					var compatibility = copyProperty(this, property, property.recordUpdate)
					if (changedCode)
						compatibility = 2
					if (compatibility > 0) {
	...
	function copyProperty(source, target, freezeTarget, startingIndex) {
		...
		if (freezeTarget) {
			target.metadata = UNSTRUCTURED_MARKER
...
// TODO: upgrading from reference type to object type can be acceptabl
				compatibility = 2
			}
			if (!targetChild) {
				var targetChild = []
...
// TODO: Check to make sure the values are compatible
		...
		if (target.recordUpdate && target.length > source.length) {
			// reverse freeze if the target has more values, we don't the source becoming incompatible
...
function serializeSharedProperty(sharedProperty, writers, expectsObjectWithNullKey, isRoot) {
	if (!(expectsObjectWithNullKey && sharedProperty.code === DEFAULT_TYPE)) {
		writers.writeProperty(null, isRoot ? null : sharedProperty.key, sharedProperty.type || (sharedProperty.type = types[sharedProperty.code]))
	}
