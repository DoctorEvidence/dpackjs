...
function createSharedStructure(from, options) {
	...
	sharedProperty.startWrite = function(avoidShareUpdate, value) {
		...
		if (value && value.constructor === Array) {
			...
			if (sharedProperty.code != ARRAY_TYPE)
	...
	sharedProperty.endWrite = function() {
		...
		for (let i = 0; i < activeList.length; i++) {
			...
			let score = 0
			if (activeSharedProperty.metadata !== UNSTRUCTURED_MARKER && !sharedProperty.isFrozen) {
	...
	sharedProperty.upgrade = function(property, randomAccess) {
		...
		if (property) {
			...
			if (property.blockStructure === blockShared && property.blockVersion === blockShared.version &&
						property.blockStructure !== property) {
					// but if the version incremented, we need to update
				return 0
			}
			var changedCode
			if (blockShared.code !== property.code)
				changedCode = true
			var compatibility = copyProperty(blockShared, property)
			if (changedCode)
				compatibility = 2
...
function makeShared(thisProperty, activeList, needsCleanup) {
	...
	sharedProperty.values = {indexOf() { return -1 }, length: -100}
	sharedProperty.newProperty = function(instance) {
		return makeShared(instance, activeList, needsCleanup)
	...
	sharedProperty.getProperty = function(value, key, type, extendedType, writeProperty, writeToken, lastPropertyIndex) {
		...
		if (property) {
			...
			if (!sharedProperty.isFrozen && !currentAvoidShareUpdate &&
				(typeof key === 'string' || key === null) && sharedProperty.length < 512 && type != 'unshared') {
