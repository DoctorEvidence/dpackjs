...
function createSharedStructure(from, options) {
	...
	sharedProperty.startWrite = function(avoidShareUpdate) {
	...
	// return true if the property was upgraded, or is compatible,
	// false if the shared information needs to be written out, or reserialized
		...
		if (compatibility >= 0) {
			...
			if (property.recordUpdate) {
				if (compatibility == -1) {
					...
					// incompatible, reserialize
					return false
				}
			} else if (blockShared.length > 0) {
				// not a shared property, (but block is shared) so write shared block and record the new property reference
				return false
			}
		}
	}
