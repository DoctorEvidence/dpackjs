...
function createSharedStructure(onUpdate) {
	...
	sharedProperty.reset = () => {
		for (let i = 0; i < activeList.length; i++) {
			let childSharedProperty = activeList[i]
			childSharedProperty.active = false
			let instance = childSharedProperty.instance
			...
			if (instance.values && instance.values.length > 0) {
				childSharedProperty.previousValues = instance.values
				instance.values = []
