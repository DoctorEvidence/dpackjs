...
/* The ascii variant
	...
	function extendedNamedProperty(reader) {
		var readProperty = namedProperty(reader)
		return function(number) {
			var property = readProperty(number)
			var Class = options.readerByType[readObject().type]
			var readParsedValue = property.readValue
			property.readValue = function() {
				var properties = readParsedValue()
	...
	function readDate() {
		return new Date(readOpen())
	}

	function readLengthDefinedBlock() {
		readNumber() // don't do anything with it in this mode
		return readOpen()
	}

	function readIdentifiedBlock() {
		var parentReferenceableValues = referenceableValues
		var id = readOpen()
		try {
			assignValue(beginBlock())
		} catch (error) {
			if (error.message == 'BUFFER_SHORTAGE') {
				if (error.whenResumed) {
		...
		function assignValue(value) {
			readOpen() // should be an end-block token
