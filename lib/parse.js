...
/*					if (!property) {
	...
	var parser = {
		...
		read: read,
		readDeferreds: function() {
			if (deferredReads) {
				var deferredRead
				while (deferredRead = deferredReads.shift()) {
					var target = deferredRead.value
					var result = readSequence(1, [{
						resume: true,
