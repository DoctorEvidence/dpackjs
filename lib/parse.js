...
/* The ascii variant
	...
	defaultMethods[1] = readArray(readAsDefault)
	readAsDefault.readTypedElements = false

	function typedArray(length) {
		if (length === 0) {
			return null // typed array of length 0 always means null
		}
		var reader = readType() // read the type
		if (reader.readTypedElements) {
			reader.readTypedElements(length - 1) // let the element reader read the remaining elements
		} else if (length === 2) {
			reader() // just read the next element
		} else {
			throw new Error(reader.name + ' can not handle multiple values')
		}
	}
	...
	readAsString.readTypedElements = false
	...
	readAsNumber.readTypedElements = false
	numberMethods[1] = readArray(readAsNumber)
	...
	var typeMethods = [
		...
		function(length) {
			var structure = []
			// TODO: if length == 1 then array of?
			...
			function readObjectElements(length) {
	...
	var readType = readNext(propertyMethods)
	...
	function read() {
		try {
			var lastRead = offset
			var result = readOpen()
			while (unfulfilledReferences > 0) {
				lastRead = offset
				readOpen() // read any referenced objects
			}
		} catch(error) {
