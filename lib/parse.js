	...
	function readSequence(length, thisProperty) {
		...
		if (thisProperty.resume) { // resuming, thisProperty is the resume state.
			...
			if (previousProperty) {
				...
				previousProperty.reader ? previousProperty.reader(previousProperty) : readSequence(previousProperty.length, previousProperty)
			}
			i = thisProperty.i
		...
		for (; i < length;) {
			...
			if (property && property.type === 2/*REFERENCING_TYPE*/ && propertyState < 2) {
				var values = property.values
				if (property.block !== currentBlock && !(type === 3 && number >= 4)) {
					backupState(property)
					values = values.slice(0) // copy this
					values.index = property.values.index
			...
			if (type === 2 /*STRING_CODE*/) {
				...
				if (number > 10) {
					if (number === 11) {
						...
						value = readAsBlock(thisProperty)
