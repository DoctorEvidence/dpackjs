		...
		for (; i < length;) {
				...
				if (number < 6) {
					...
					if (number <= NUMBER_TYPE) {
						...
						if (property) {
							...
							if (thisProperty.recordParse) {
								if (propertyIndex >= 512)
									thisProperty.recordParse(thisProperty)
								else
									throw new Error('Attempt to read property that alters shared structure')
							}
							property = thisProperty[propertyIndex] = []
						}
						property.key = null
				...
				if (type === 2 /*STRING_CODE*/) {
					...
					if (number > 13) {
						if (number === END_SEQUENCE)
							...
							if (!options.forDeferred || options.parseDeferreds) {
						...
						if (propertyState > EXTENSIONS) {
							if (propertyState === METADATA_TYPE)
			...
			if (propertyState < 2 && property && property.code === REFERENCING_TYPE/*REFERENCING_TYPE*/) {
				...
				if (typeof value === 'number') {
					...
					if (values.nextPosition > -1 && values.nextPosition !== null) {
...
/*					if (!property) {
	...
	function read(property) {
		...
		while (true) {
			...
			var sharedBlock = targetProperty.blockStructure
			var result
			function readBlock() {
				result = readSequence(1, property = [{
					resume: true,
					thisProperty: sharedBlock || targetProperty,
					object: target
				}])[0]
			}
			if (sharedBlock)
				sharedBlock.readingBlock(readBlock)
			else
				readBlock()
...
exports.parse = function(stringOrBuffer, options) {
	...
	if (options && options.shared && options.shared.readingBlock) {
		return options.shared.readingBlock(function() {
...
exports.parseLazy = function(buffer, options) {
	...
	} else if (options) { // otherwise, may be a primitive, proactively parse
		return createParser(options).setSource(buffer.toString()).read()
	} else {
