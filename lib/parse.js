...
function createParser(options) {
	...
	function readProperty(reference) {
		...
		try {
			if (reference < 6) {
				...
				if (key !== null) { // TODO: When exactly do we increment? (I think only if referrer type is child or next)
	...
	function readArray(readValue, transform) {
		return function readArray(length, startingArray) {
			var array = startingArray || []
			var arrayProperty = property
			var elementsProperty = property.child || property
			if (length > 10) {
				if (length === 13) {
					return END_SEQUENCE
				}
			...
			try {
				...
				handlePossiblePause(error, {
					...
					resumed: function(value) {
						...
						property = arrayProperty
						return readArray(length - array.length, array)
					}
				})
			}
		}
