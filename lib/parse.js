...
function createParser(options) {
	...
	function readSequence(length, object, isArray, thisProperty, previousProperty) {
		if (length > 10) {
			if (length === 11) {
				length = MAX_LENGTH
			} else if (length === 14) {
			...
			for (var i = 0; i < length;) {
				...
				if (type > 1) {
					if (type === 2 /*STRING_CODE*/) {
						...
						if (property.type === NUMBER_TYPE && propertyState === 0) {
							value = +value
						}
					} else { /*NUMBER_CODE*/
						if (number >= 4) {
					...
					value = readSequence(number, property.type === ARRAY_TYPE ? [] : {}, property.type === ARRAY_TYPE, property)
					if (value === PAUSED) {
				...
				if (property) {
					...
					if (property.readValue) {
						value = property.readValue(value)
					}
					if (isArray) {
						if (property.key != null && thisProperty.child !== thisProperty && property.index !== 7) {
	...
	/*					if (!property) {
			...
			handlePossiblePause(error, {
				...
				resume: function(value) {
					...
					return readSequence(length - i, object, i === 0 ? undefined :
