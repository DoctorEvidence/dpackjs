...
function createParser(options) {
	...
	function readProperty(reference) {
		...
		if (reference < 8) {
			...
			} else if (reference === BLOCK_TYPE) {
				// readingProperty.key is the length of the block. In this mode, we ignore it and just keep reading
				readingProperty.key = undefined
			}
