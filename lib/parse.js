...
/* The ascii variant
	...
	function readExtended() {
		var className = readOpen()
		var classConverter = classByName.get(className)
		if (!classConverter && options.throwOnMissingClass) {
...
function readMap(keysAndValues) {
		...
		parser.onResume = function(updatedString) {
			try {
				if (incomingOnResume) {
					...
					incomingOnResume(updatedString)
				} else {
