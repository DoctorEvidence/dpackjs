...
function createParser(options) {
	...
	function arrayToObject(length, object, isArray, thisProperty, previousProperty) {
		if (length > 10) {
			if (length === 11) {
				length = MAX_LENGTH
			} else if (length === 14) {
			...
			for (var i = 0; i < length;) {
				...
				switch(type) {
					...
					case SEQUENCE_CODE:
						...
						value = arrayToObject(number, property.type === ARRAY_TYPE ? [] : {}, property.type === ARRAY_TYPE, property)
						if (value === PAUSED) {
						...
						if (number < 6) {
							if (number === METADATA_TYPE) {
								...
								if (propertyState === 0) {
									...
									property = {
										readValue: false,// TODO: is this faster?
				...
				if (property) {
					if (property.type === REFERENCING_TYPE) {
	...
	/*					if (!property) {
						...
						if (key)
							...
							if (key === null) {
								if (isArray) {
									object.push(value)
								} else { // for objects null indicates replacing the parent object
									object = value
								}
