...
var REFERRER_CREATED_PROPERTY = 4
...
function createParser(options) {
	...
	function readProperty(reference) {
		...
		try {
			...
			if (referrerType === REFERRER_CHILD) {
				...
				// ...
			} else if (referrerType === REFERRER_CREATED_PROPERTY) {
				// copy everything except key
				referrerProperty.readValue = readingProperty.readValue
				referrerProperty.child = readingProperty.child
				referrerProperty.next = readingProperty.next
				referrerProperty.values = readingProperty.values
			...
			if (reference < 6) {
				referrerType = REFERRER_CREATED_PROPERTY
			} else {
			...
			var errorHandler = {
				...
				resume: function(value) {
					if (readingProperty) {
						property = readingProperty
						try {
							return value || readingProperty.readValue() // reading this can change the current property
						} catch(error) {
	...
	function readAsIdentifiable(reference) {
		lastRead = offset
		try {
			return objects[reference] = readAsDefault()
		} catch(error) {
			handlePossiblePause(error, {
				resume: function(value) {
					if (value) {
	...
	function read(result) {
		try {
			...
			if (firstToken === 94 || firstToken === 95) {
