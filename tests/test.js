...
suite('dpack basic tests', function(){
	...
	test.only('findShared', function() {
		...
		var sharedSerialized
		var sharedStructure = createSharedStructure(function(serialized) {
		...
		var parsed = parse(serializedWithShared, { properties: [sharedStructure] })
		assert.deepEqual(parsed, testData[0])
		var parsed = parse(serializedWithShared1, { properties: [sharedStructure] })
		assert.deepEqual(parsed, testData[1])
		var parsed = parse(serializedWithShared2, { properties: [sharedStructure] })
		assert.deepEqual(parsed, testData[2])
		
		// reset
		sharedStructure = createSharedStructure(function(serialized) {
			console.log('shared structure updated', serialized)
			sharedSerialized = serialized
		})
		parse(sharedSerialized, { properties: [sharedStructure] })
		var parsed = parse(serializedWithShared, { properties: [sharedStructure] })
		assert.deepEqual(parsed, testData[0])
		var parsed = parse(serializedWithShared1, { properties: [sharedStructure] })
		assert.deepEqual(parsed, testData[1])
